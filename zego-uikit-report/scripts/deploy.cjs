...
// 读取 dist 参数, 如果没有传, 就默认用 workspace
// 例如: --dist=./dist
const distArg = process.argv.find(arg => arg.startsWith('--dist='))
const dist = distArg ? distArg.split('=')[1] : path.join(workspace, '..');
console.log('[*] Dist:'+ dist)

const sourceDir = path.join(workspace, '.');
const distDir = path.join(dist, 'uni_modules', projName);
