...
// Android权限查询
async function requestAndroidPermission(permissionID: string): Promise<{ code: number; message: string; }> {
    return new Promise((resolve, reject) => {
        plus.android.requestPermissions(
            [permissionID], // 理论上支持多个权限同时查询，但实际上本函数封装只处理了一个权限的情况。有需要的可自行扩展封装
            function (resultObj: { granted: string[]; deniedPresent: string[]; deniedAlways: string[]; }) {
                let result = 0;
                let message = '';
                for (let i = 0; i < resultObj.granted.length; i++) {
                    const grantedPermission = resultObj.granted[i];
                    message = '已获取的权限：' + grantedPermission
                    result = 1
                }
                for (let i = 0; i < resultObj.deniedPresent.length; i++) {
                    const deniedPresentPermission = resultObj.deniedPresent[i];
                    message = '拒绝本次申请的权限：' + deniedPresentPermission
                    result = 0
                }
                for (let i = 0; i < resultObj.deniedAlways.length; i++) {
                    const deniedAlwaysPermission = resultObj.deniedAlways[i];
                    message = '永久拒绝申请的权限：' + deniedAlwaysPermission
                    result = -1
                }
